...
- **双排序互补采样**：时间流会漏排序靠后的真实用户帖（求避雷/比价帖），每日采集对品牌词额外补抓 1 页 `general` 排序（`SEARCH_GENERAL_PAGES_BY_CATEGORY`，竞品/行业词不补）。注意：被平台搜索端压制的避雷帖两个排序都捞不到（已实测），这类只能靠人工发现后补录。历史回填用 `backend/temp/backfill_brand.py [关键词] [页数] [排序]`。
