...
function CommentPanel({ noteId }: { noteId: string }) {
  ...
  if (error) {
    return <p className="pt-3 text-xs text-[#7b8494]">评论加载失败</p>;
  }

  if (!comments || comments.length === 0) {
    return <p className="pt-3 text-xs text-[#7b8494]">暂无评论数据</p>;
  }

  return (
    <ul className="mt-3 space-y-2 border-t border-[#eaeef4] pt-3">
      {comments.map((c) => (
        <li key={c.comment_id} className="flex items-start gap-2 text-xs">
          <span className="shrink-0 font-medium text-[#7b8494] w-20 truncate">
            {c.author.nickname}
          </span>
          <span className="flex-1 text-[#5a6474] leading-relaxed">{c.content}</span>
          <div className="flex items-center gap-2 shrink-0">
            {c.likes > 0 && (
              <span className="flex items-center gap-0.5 text-[#727171]">
            ...
            {c.sentiment ? (
              ...
              <span className="text-[#c7cdd8] italic">待分析</span>
            )}
          </div>
        </li>
      ))}
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {note.stats.comments > 0 && (
                      expanded
                        ? <ChevronUp size={14} className="text-[#727171]" aria-hidden="true" />
                        : <ChevronDown size={14} className="text-[#727171]" aria-hidden="true" />
                    )}
                  </div>
                </div>
